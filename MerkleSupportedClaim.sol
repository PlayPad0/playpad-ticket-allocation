...
contract PlayPadAltavaClaim is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    */
    ...
    function changeTotalSellAmount(uint256 _value)
        external
        onlyOwner
        nonReentrant
    {
        totalSellAmountToken = _value;
    }

    function changeSaleTokenAddress(address tokenAddress)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function isUserWhitelisted(
        ...
        bool isWhitelisted = MerkleProof.verify(_proof, MERKLE_ROOT, _userLeaf);
    ...
    function withdrawTokens() external nonReentrant onlyOwner {
        uint256 totalTokenValue = saleToken.balanceOf(address(this));
        saleToken.transfer(msg.sender, totalTokenValue);
    }
    ...
    function claimTokens(uint256[4] calldata _data, bytes32[] memory _proof)
        external
        nonReentrant
    {
        bytes32 userLeaf = keccak256(abi.encodePacked(msg.sender, _data));
        ...
        require(_data[0] != 0, "user is not allowed");
        ...
        require(
            _data[1] >=
                investor.claimedValue.add(
                    _data[1].mul(roundDetail.roundPercent).div(10000000)
                ),
